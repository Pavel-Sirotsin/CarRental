...
public class ShowAdminUserTable implements Command {
    private static final Logger logger = LogManager.getFormatterLogger(ShowAdminUserTable.class);

    @Override
    public void execute(HttpServletHandler handler) throws ServletException, IOException {
        logger.traceEntry("- try to forward to the admin user table.");
