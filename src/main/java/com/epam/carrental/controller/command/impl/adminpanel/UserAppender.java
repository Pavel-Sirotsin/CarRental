...
public class UserAppender implements Command {
    private static final Logger logger = LogManager.getFormatterLogger(UserAppender.class);
    ...
    public void execute(HttpServletHandler handler) throws ServletException, IOException {
        logger.traceEntry("- try to add a new user from admin user table.");
        ...
        logger.debug("line(%d), Session user empty: %s", 34, new ObjectMessage(sessionUser));
        ...
        if (sessionUser == null) {
            ...
            String role = handler.getParameter(ParameterManager.USER_ROLE_PARAMETER);
            ...
            userDTO.role = UserRole.valueOf(role.toUpperCase());
            userDTO.account = accountDTO;
            logger.debug(new ObjectMessage(userDTO));
