...
public class ShowBookingResultPage implements Command {
    private static final Logger logger = LogManager.getFormatterLogger(ShowBookingResultPage.class);

    @Override
    public void execute(HttpServletHandler handler) throws ServletException, IOException {
        logger.traceEntry("- try to forward to the booking result page.");
        ...
        if (booking == null) {
            ...
            handler.forward(PageManager.BOOKING_RESULT_PAGE_PATH);
        }
