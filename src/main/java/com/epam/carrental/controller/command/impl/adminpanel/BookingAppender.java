...
public class BookingAppender implements Command {
    private static final Logger logger = LogManager.getFormatterLogger(BookingAppender.class);
    ...
    public void execute(HttpServletHandler handler) throws ServletException, IOException {
        logger.traceEntry("- try to add a new order from booking table.");
        ...
        if (sessionUser == null) {
            ...
            try {
                bookingServiceImpl.createByUserAndCarId(params);
